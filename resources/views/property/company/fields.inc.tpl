...
<div class="form-group">
	...
	<div class="col-md-9">
		<select type="text" id="tag_ids" name="tag_ids[]" class="form-control select-model" data-params='{"f[type]":"1"}' data-model="property/tag" data-text="{{tag_name}}" data-term="{{tag_name}}" data-placeholder="请输入标签..." value="<{if !empty($_data->tags)}><{$_data->tags->pluck('id')->toArray()|implode:','}><{/if}>" multiple="multiple"></select>
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-md-9">
		<select id="fids" name="fids[]" class="select-model form-control" data-model="property/floor"<{if $_data.oid}> data-params='{"f[oid][eq]":"<{$_data.oid}>"}'<{/if}> data-text="{{name}}" data-term="{{name}}" data-placeholder="请输入该楼名." value="<{if !empty($_data->floors)}><{$_data->floors->pluck('id')->toArray()|implode:','}><{/if}>" multiple="multiple"></select>
	</div>
</div>
<div class="form-group">
