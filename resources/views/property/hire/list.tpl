...
<{block "filter"}>
<{include file="property/hire/filters.inc.tpl"}>
<{/block}>
...
<td data-from="floor" data-orderable="false">
	<a href="<{'property/floor?f[id][eq]='|url}>{{full.floor.id}}"><span class="label label-primary">{{data.name}}</span></a>
</td>
...
{{if full.status!=-1}}<a href='<{''|url}>/<{block "namespace"}>property<{/block}>/hire/toggle/{{full.id}}'>{{data}}</a>{{else}}{{data}}{{/if}}
</td>
