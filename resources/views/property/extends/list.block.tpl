	...
	<{block "menus"}>
		...
		<{* <div class="collapse navbar-collapse pull-left">
			<ul class="nav navbar-nav">
				...
				<{block "menus-create"}>
				<li>
					<a href="<{''|url}>/<{block "namespace"}>property<{/block}>/<{block "name"}><{/block}>/create" data-shortcuts="ctrl+n"> <i class="fa fa-plus"></i> 添加</a>
				</li>
				<{/block}>
...
<div class="table-responsive">
	...
	<div class="block-options" id="tools-contrainer">
		...
		<a class="btn btn-success btn-sm btn-alt" data-toggle="tooltip" title="新建<{block 'title'}><{/block}>" href="<{''|url}>/<{block "namespace"}>property<{/block}>/<{block "name"}><{/block}>/create" data-shortcuts="ctrl+n"><i class="fa fa-plus animated pulse infinite"></i></a>
		<{/block}>
			...
			<ul class="dropdown-menu dropdown-custom dropdown-menu-left">
				...
				<{block "table-tools-dropdown-operate"}>
					...
					<li>
						...
						<a href="<{''|url}>/<{block "namespace"}>property<{/block}>/<{block "name"}><{/block}>/0" method="delete" selector="#datatable [name='id[]']:checked" confirm="<{block "table-tools-dropdown-delete-confirm"}>您确定删除这%L项？此操作不可恢复！<{/block}>" class="" data-shortcuts="del"><span class="text-danger"><i class="fa fa-times pull-right "></i>删除所选</span></a>
						<{/block}>
				...
				<{block "table-tools-dropdown-export"}>
					...
					<li>
						...
						<a href="javascript:void(0)" data-namespace="<{block "namespace"}>property<{/block}>" data-name="<{block 'name'}><{/block}>" data-export="csv"><i class="fi fi-csv pull-right"></i> CSV </a>
						<a href="javascript:void(0)" data-namespace="<{block "namespace"}>property<{/block}>" data-name="<{block 'name'}><{/block}>" data-export="pdf"><i class="fi fi-pdf pull-right"></i> PDF</a>
						<a href="javascript:void(0)" data-namespace="<{block "namespace"}>property<{/block}>" data-name="<{block 'name'}><{/block}>" data-export="xls"><i class="fi fi-xls pull-right"></i> Excel 2003</a>
						<a href="javascript:void(0)" data-namespace="<{block "namespace"}>property<{/block}>" data-name="<{block 'name'}><{/block}>" data-export="xlsx"><i class="fi fi-xlsx pull-right"></i> Excel 2007+</a>
					</li>
				<{/block}>
	...
	<table id="datatable" class="table table-vcenter table-condensed table-bordered table-striped table-hover"
		...
		data-namespace='<{block "namespace"}>property<{/block}>'
			...
			<{block "table-td"}>
				...
				<td class="text-center" data-from="" data-orderable="false">
					<div class="btn-group">
						...
						<a href="<{''|url}>/<{block "namespace"}>property<{/block}>/<{block "name"}><{/block}>/{{full.id}}/edit" data-toggle="tooltip" title="编辑" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
							<{/block}>
							<{block "table-td-options-plus"}><{/block}>
							<{block "table-td-options-delete"}>
						<a href="<{''|url}>/<{block "namespace"}>property<{/block}>/<{block "name"}><{/block}>/{{full.id}}" method="delete" confirm="<{block "table-td-options-delete-confirm"}>您确定删除这项：{{full.id}}吗？<{/block}>" data-toggle="tooltip" title="删除" class="btn btn-xs btn-danger"><i class="fa fa-times"></i></a>
							<{/block}>
