...
<form action="<{'admin'|url nofilter}>/<{block "name"}><{/block}>" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		...
		<div class="col-md-9">
			<div class="input-group">
				<select id="status" name="f[status][eq]" class="form-control" placeholder="请选择审核状态.">
					<option value="0"<{if $_filters.status.eq == 0}> selected="selected"<{/if}>>未审核</option>
					<option value="1"<{if $_filters.status.eq == 1}> selected="selected"<{/if}>>审核通过</option>
					<option value="2"<{if $_filters.status.eq == 2}> selected="selected"<{/if}>>审核不过</option>
				</select>
			</div>
		</div>
	</div>
