...
<form action="<{'admin'|url nofilter}>/<{block "name"}><{/block}>" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		...
		<div class="col-md-9">
			<select type="text" id="tag_id" name="q[ofTag]" class="form-control select-model" data-params='{"f[type]":"0"}' data-model="admin/tag" data-text="{{tag_name}}" data-placeholder="请输入标签..." value="<{$_queries.ofTag}>"></select>
		</div>
	</div>
	<div class="form-group col-sm-4">
