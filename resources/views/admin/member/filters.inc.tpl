...
<form action="<{'admin'|url nofilter}>/<{block "name"}><{/block}>/" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		...
		<div class="col-md-9">
			<select type="text" id="realname" name="filters[role_id]" class="form-control select-model" data-model="admin/role" data-id="{id}" data-text="{display_name}({name})" data-placeholder="请输入关键词..." value="<{$_filters.role_id.equal}>"></select>
		</div>
	</div>
