...
<form action="<{'admin'|url}>/<{block "name"}><{/block}>/" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		...
		<{foreach $_fields.gender as $v}>
			<label class="radio-inline">
				<input type="radio" name="gender" value="<{$v.id}>" <{if $_filters.gender.equal == $v.id}>checked="checked"<{/if}> > <{$v.text}>
			</label>
		<{/foreach}>
		</div>
	</div>
	...
	<div class="form-group form-actions col-sm-4 pull-right text-right">
