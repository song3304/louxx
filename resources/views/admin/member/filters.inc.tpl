...
<form action="<{'admin'|url nofilter}>/<{block "name"}><{/block}>/" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		...
		<div class="col-md-9">
			<select type="text" id="role_id" name="filters[role_id]" class="form-control tree-model" data-model="admin/role" data-text="{{display_name}}({{name}})" data-placeholder="请输入关键词..." value="<{$_filters.role_id.equal}>"></select>
		</div>
	</div>
