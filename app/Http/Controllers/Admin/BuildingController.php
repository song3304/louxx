...
class BuildingController extends Controller
{
	...
	{
		$office_building = new Building;
		$builder = $office_building->newQuery()->with(['properter','pics']);
	...
	public function store(Request $request)
	{
		...
		$pic_ids = $data['pic_ids'];unset($data['pic_ids']);
		$tag_ids = $data['tag_ids'];unset($data['tag_ids']);//标签 
	...
	{
		$office_building = Building::with(['pics'])->find($id);
		if (empty($office_building))
			return $this->failure_noexists();
